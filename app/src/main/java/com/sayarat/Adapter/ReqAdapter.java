...
public class ReqAdapter extends RecyclerView.Adapter<ReqAdapter.ImageAdapterViewHolder>{
    ArrayList<ProductModel> data  ;
    LayoutInflater layoutInflater ;
    Activity context ;
    req_listener req_listener;

    public ReqAdapter(ArrayList<ProductModel> dta, Activity context , req_listener req_listener) {
        this.data = dta;
        this.req_listener = req_listener;
    ...
    public ImageAdapterViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View item_view = layoutInflater.inflate(R.layout.request_item,parent , false ) ;
    ...
    class  ImageAdapterViewHolder extends RecyclerView.ViewHolder{
       ImageView profileImg ;
        ...
        public ImageAdapterViewHolder(final View itemView) {
            super(itemView);
            profileImg = (ImageView) itemView.findViewById(R.id.req_img);
            title = (TextView) itemView.findViewById(R.id.req_title);
            time = (TextView) itemView.findViewById(R.id.req_time);
            reqs_con = itemView.findViewById(R.id.reqs_con);

            profileImg.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    UserModel current = data.get(getAdapterPosition()).getUserModel();
                    // start detailes
                    req_listener.start_profile(current,itemView);
                }
            });
            reqs_con.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    req_listener.start_detailes(data.get(getAdapterPosition()),view );
                }
            });
